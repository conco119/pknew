...
function setPrice(obj) {
    ...
    $.post('./admin?mc=helps&site=ajax_get_number_format', { 'value': value }).done(function(data) {
        $(obj).val(data);
    });
}
...
function LoadDeleteItem(id, mc) {
    PNotify.removeAll();
    $("#DeleteItem").attr("onclick", `DeleteItem(${id}, '${mc}')`);
}

function DeleteItem(id, mc) {
    $.post("./admin?mc=" + mc + "&site=ajax_delete", { 'id': id })
        .done(function(data) {
            if (data == 0) {
