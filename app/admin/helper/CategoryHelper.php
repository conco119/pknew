...
class CategoryHelper extends HelpAbstract
{
    ...
    {
        $cats = $this->pdo->fetch_all("SELECT * FROM $this->table WHERE id <> '$cat_id' and status = 1 and parent_id = 0");
    ...
    {
        $rows = $this->pdo->fetch_one("SELECT max(id) as id FROM categories");
